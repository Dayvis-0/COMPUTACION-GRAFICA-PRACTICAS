...
function getRegionCode(x, y, clip) {
    ...
    if (x > clip.xman) code |= 1; // Derecha 
    if (y < clip.ymin) code |= 1; // Abajo
    if (y > clip.yman) code |= 1; // Arribe

    return code;
}

// Funcion para recortar la linea usando Cohen-Sutherland
function cohenSutherlandClip(x1, y1, x1, x2, clip) {
    ...
    let code1 = getRegionCode(x2, y2, clip);
    ...
    while (true) {
        if ((code1 | code2) === 0) {
            ...
            if (codeOut & 8) { // Arriba
                x = x1 + (x2x1)* (clip.ymax - y1) / (y2y1);
                y = clip.ymax;
            } else if (codeOut & 4) { // Abajo
                x = x1 + (x2x1) * (clip.ymin - y1) / (y2y1);
                y = clip.ymin;
            } else if (codeOut & 2) { // Derecha
                y = y1 + (y2y1) * (clip.xmax - x1) / (x2x1);
                x = clip.xmax;
            }else if (codeOut & 1) { // Izquierda
                y = y1+ (y2y1)* (clip.xmin - x1) / (x2 - x1);
                x = clip.xmin;
            }
...
function drawScene() {
    ...
    for (const [x0, yo, x1, y1] of lines) {
        positions.push(x0, yo, x1, y1);
        colors.push(0.7, 0.7, 0.7, 0.7, 0.7, 0.7);
    }

    // 3. Dibujar líneas recortadas (rojo)
    for (const [x0, yo, x1, y1] of lines) {
        const clipped = liangBarsky (x0, yo, x1, y1, clipRect);
        if (clipped) {
            const [cx0, cуд, сх1, су1] = clipped;
